...
The result by calling "print(fit_lm)":
```{r}
print(fit_lm)
```


The result by calling "print.blblm(fit_blblm)":

```{r}
print.blblm(fit_blblm)
```
...
The result by calling "sigma.blblm(fit_blblm)":

```{r}
sigma.blblm(fit_blblm)
```


3. coef() vs. coef.blblm() -- obtain the coefficients of the fitted model

The result by calling "coef(fit_lm)":

```{r}
coef(fit_lm)
```

The result by calling "coef.blblm(fit_blblm)":

```{r}
coef.blblm(fit_blblm)
```

4. confint() vs. confint.blblm() -- obtain the confidence intervals for the parameters of the fitted model

The result by calling "confint(fit_lm)":

```{r}
confint(fit_lm)
```

The result by calling "confint.blblm(fit_blblm)":

```{r}
confint.blblm(fit_blblm)
```

5. predict() vs. predict.blblm() --  model predictions for the fitted model

The result by calling "predict(fit_lm, newdata = data.frame(wt = c(2.5, 3), hp = c(150, 170)))":

```{r}
predict(fit_lm, newdata = data.frame(wt = c(2.5, 3), hp = c(150, 170)))
```

The result by calling "predict.blblm()":

```{r}
predict.blblm(fit_blblm, new_data = data.frame(wt = c(2.5, 3), hp = c(150, 170)))
```

As we can observe from the results above, blblm() is essentially the BLB of lm() with similar affiliated functions but has better performance in terms of lower estimated standard deviation of the errors (what we get by implementing sigma() and sigma.blblm() above, 2.152752 for fit_lm and 1.50569 for fit_blblm). 
...
The result by calling "print.blblogreg(fit_blblogreg)":

```{r}
print.blblogreg(fit_blblogreg)
```

2. sigma() vs. sigma.blblogreg() -- compute the residual standard deviation (sigma) of the logistic model

The result by calling "sigma(fit_glm)":
...
The result by calling "sigma.blblogreg(fit_blblogreg)":

```{r}
sigma.blblogreg(fit_blblogreg)
```


3. coef() vs. coef.blblogreg() -- obtain the coefficients of the logistic model

The result by calling "coef(fit_glm)":
...
```{r}
coef.blblogreg(fit_blblogreg)
```

4. confint() vs. confint.blblogreg() -- obtain the confidence intervals for the parameters of the logistic model

The result by calling "confint(fit_glm)":

```{r}
confint(fit_glm)
```

The result by calling "confint.blblogreg(fit_blblogreg)":

```{r}
confint.blblogreg(fit_blblogreg)
```
...
The result by calling "predict.blblogreg()":

```{r}
predict.blblogreg(fit_blblogreg, new_data = data.frame("Lag1" = c(0.382, 0.4, 0.5),"Lag2" = c(1.03, 1.4, 0.5), "Volume" = c(1.2, 1.34, 1.1)))
```
